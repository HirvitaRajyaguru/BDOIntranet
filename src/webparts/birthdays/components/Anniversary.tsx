...
export default class Anniversary extends React.Component<IBirthdaysProps, IBirthdayState> {
      ...
      .get((error: any, city: any, rawResponse?: any) => {
        ...
        this.props.msGraphClient.api('/me/department')  //get current user's department
          .get((ServiceLineerror: any, sericeLine: any, ServiceLinerawResponse?: any) => {
            this.GetItemsForAnniversary(city.value, sericeLine.value);
          });
      });
  }

  //Will Fetch the Birthday data from EmployeeMaster Sharepoint List
  public GetItemsForAnniversary(location, serviceLine) {
    ...
    newCommonObj.getDataFromListUsingGuid(this.props.siteurl, this.props.listId, null, method).then(async (res) => {
      if (res.data.value != undefined && res.data.value != null) {
        ...
        var serviceVise;
        var dataFiltered;
        dataFiltered = userData.filter(data => new Date(data.JoiningDate).getDate() == new Date().getDate()
          && new Date(data.JoiningDate).getMonth() == new Date().getMonth()
          && new Date(data.JoiningDate).getFullYear() != new Date().getFullYear());
        if (dataFiltered.length > 0) {
          if (this.props.serviceline == strings.allServiceLine || this.props.location == strings.allLocation) //If in Property Pane selected is All Location or All Serviceline
          {
            ...
            {
              dataFiltered = dataFiltered.filter(data => new Date(data.JoiningDate).getDate() == new Date().getDate()
                && new Date(data.JoiningDate).getMonth() == new Date().getMonth()
                && new Date(data.JoiningDate).getFullYear() != new Date().getFullYear());
              serviceVise = dataFiltered.filter(
          ...
          ////Get Like count from AnniversaryLikeCount list
          await this.getLikeCount(this.state.items);

          let commonObj: Common = new Common();
          var items = this.state.items;
          ////Get Bithday is-liked or not by current user for all Anniversary Persons in slider
          await commonObj.getDataFromList(this.props.siteurl, strings.AnniversaryLikeCountList, '', "Get Anniversary Like Count").then(async (res) => {
            for (let i = 0; i < items.length; i++) {
              let userId = await commonObj.getUserIdByEmail(items[i].Email, this.props.siteurl);//Anniversary Person 
  ...
  private async getLikeCount(bdayUsers) {
    ...
    for (let i = 0; i < bdayUsers.length; i++) {
      ...
      await commonObj.getDataFromList(this.props.siteurl, strings.AnniversaryLikeCountList, likeCountQuery, "Get Like Count from List").then(async (resLikedItems) => {
      ...
      <div className={styles.birthdays} >
        <div className={styles.containertab}>
          {(this.state.isLoading) ? <Loader message="Please wait..." /> :
            (this.state.items[0].Display === "No Anniversary today")
              ?//If No Anniversary are there it will show message of-"No Anniversary today"
              <div>
                <Persona
              ...
              ://If any Anniversary date is matched with today's date will Load the Anniversary details of person and show in webpart
          ...
          <Link href={`${this.props.siteurl}/SitePages/AnniversaryWishes.aspx`} target='_blank' data-interception="off" styles={siteTextStyles}><Icon iconName="BirthdayCake" className={iconclassNames.birthdaywish} /></Link>
        </div>
      </div>
    );
  }
  //By clicking LikeButton will store data of BirthdayPerson,LikedBy person  with LikeFlag value true and unlike will set LikeFlag value false in in AnniversaryLikeCount sharepoint list
      ...
      await this.GetLikeFlag(userEmail, this.props.currentUserEmail).then(res => {
        if (res.length > 0) {
          res.forEach(element => {
            ...
            //sp.web.lists.getByTitle(strings.AnniversaryLikeCountList).items.getById(id).update({
            this.web.lists.getByTitle(strings.AnniversaryLikeCountList).items.getById(id).update({
              'LikeFlag': true,
            });
          });
          ...
          this.web.lists.getByTitle(strings.AnniversaryLikeCountList).items.add({
            LikeFlag: true,
  ...
  private GetLikeFlag(empEmail, currentUserEmail): Promise<any> {
    ...
    return commonObj.getDataFromList(this.props.siteurl, strings.AnniversaryLikeCountList, isLikedQuery, "Get Liked or not from List").then(async (resLikedItems) => {
      return resLikedItems.data.value;
    });
  }
  //This will store Details of Bdyperson Name ,Wished By,BdayMessage in AnniversaryWishes sharepoint List
  public async SendData(BirthdayPerson, userEmail, msg, e) {
    ...
    if (msg != null && res != (e.which == 32)) {//message should not be null or First charactor of message should not be space 
      this.web.currentUser.get().then((r) => {
        ...
        this.web.lists.getByTitle("AnniversaryWishes").items.add({
